...
export function NotificationProvider({ children }) {
    ...
    const markAllRead = async () => {
        try {
            await axios.post('https://mamamianpizza.com/api/notifications/mark-all-read', {}, {withCredentials: true} );
            setnoleidas(0);
        } catch (err){
   ...
   const fetchNoLeidas = async () => {
        try {
            const {data} = await axios.get('https://mamamianpizza.com/api/notifications/unread', {withCredentials: true});
    ...
    useEffect(() => {
        fetchNoLeidas();
        evtSourceRef.current = new EventSource('https://mamamianpizza.com/api/notifications/stream', {withCredentials: true});
