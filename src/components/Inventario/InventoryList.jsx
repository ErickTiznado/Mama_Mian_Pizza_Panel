...
import { Search, Trash2, Edit, AlertTriangle, Plus, Filter, ArrowUpDown, Package, Grid3X3, List, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
const InventoryList = ({ inventoryData, onDataUpdate, loading }) => {
  ...
  const filteredData = inventoryData
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'cantidad' || sortBy === 'cantidad_actual' || sortBy === 'costo') {
  ...
  const validateForm = () => {
    ...
    if (!formData.fecha_caducidad) {
      errors.fecha_caducidad = 'La fecha de caducidad es requerida';
    }
  ...
  const handleSave = async () => {
    ...
    try {
      const productData = {
        ...
        cantidad: parseFloat(formData.stock),
  ...
  const handleEdit = (product) => {
    setFormData({
      ...
      stock: (product.cantidad || product.cantidad_actual)?.toString() || '',
  ...
  return (
    <div className="ninv-inventory-list">
      ...
      <div className="ninv-products-container">
        {filteredData.length === 0 ? (
          <div className="ninv-empty-state">
            <Package className="ninv-empty-icon" />
                ...
                {paginatedData.map(product => (
                  <div key={product.id_ingrediente} className="ninv-product-card">
                    <div className="ninv-card-header">
                      <h3 className="ninv-card-title">{product.nombre}</h3>
                    ...
                    <div className="ninv-card-body">
                      <div className="ninv-card-info">
                        ...
                        <div className="ninv-info-row">
                          ...
                          <span className="ninv-info-value">{product.cantidad || product.cantidad_actual} {product.unidad}</span>
                        </div>
                        <div className="ninv-info-row">
                  ...
                  <tbody>
                    {paginatedData.map(product => (
                      <tr key={product.id_ingrediente}>
                        ...
                        <td>{product.cantidad || product.cantidad_actual} {product.unidad}</td>
                        <td>
                          <span className={`ninv-card-status ${product.estado?.toLowerCase() || 'normal'}`}>
                            {product.estado || 'Normal'}
                          </span>
      ...
      {showForm && (
        <div className="ninv-modal-overlay" onClick={() => {
          setShowForm(false);
          setEditingProduct(null);
          resetForm();
        }}>
